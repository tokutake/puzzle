...
//	}
	...
	switch (e.charCode) {
		...
		case 114:
			function rotate_block() {
				...
				if (y0 == y1) {
					...
					if (currentblocks[upside].x >= board.length || 
		...
		case 115:
			...
			function after_set() {
				...
				draw();
				if (erased)
					setTimeout(function () { after_set(); }, 1000);
				update_score();
			}
			set_block();
			earth_gravity();
	...
	if (currentblocks[0].x >= board.length || currentblocks[1].x >= board.length) {
		currentblocks[0].x--;
		currentblocks[1].x--;
	}
	if (currentblocks[0].y >= board[0].length || currentblocks[1].y >= board[0].length) {
		currentblocks[0].y--;
		currentblocks[1].y--;
	}
...
function kill_chains() {
	function count_chains(x, y, color, scaned_board) {
		...
		count = 1;
		return count + count_chains(x, y+1, color, scaned_board) + count_chains(x, y-1, color, scaned_board) +
			   count_chains(x-1, y, color, scaned_board) + count_chains(x+1, y, color, scaned_board);
	}
	...
	for (var x = 0; x < board.length; x++)
		for (var y = 0; y < board[0].length; y++) {
			...
			if (count >= 4) {
				for (var x = 0; x < board.length; x++)
					for (var y = 0; y < board[0].length; y++) {
						if (scaned_board[x][y] == 'scaned')
							erase_candidate[x][y] = 'erase';
					}
				erased = true;
			}
		}
